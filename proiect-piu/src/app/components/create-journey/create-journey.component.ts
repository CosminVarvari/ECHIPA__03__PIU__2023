...
export class CreateJourneyComponent implements OnInit {
  ...
  options: string[] = ['Madrid', 'London', 'Paris', 'Berlin', 'Budapest', 'Rome'];
  ...
  [
    ...
    { name: "Berlin Metropolitan Towers", price: 160, stars: 4, city: "Berlin" },
  
    { name: "Budapest Boutique Inn", price: 180, stars: 4, city: "Budapest" },
    { name: "Budapest Grandeur Lodges", price: 120, stars: 3, city: "Budapest" },
    ...
    { name: "London Splendid Residences", price: 100, stars: 3, city: "London" },
  ...
  filterCities(value: any) {
    this.filteredAccomodations = this.accomodations.filter(el => el.city === value);
  }
  ...
  selectAccommodation(accomodation: any) {
    this.selectAccommodation = accomodation;
  ...
  createJourney() {
    ...
    let destination = {name: this.destinationName.value, weather: weather[random], temperature: temperatureRandom}
    let newJourney = {destination: destination, accomodation: this.selectAccommodation};
